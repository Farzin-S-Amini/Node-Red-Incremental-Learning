...
module.exports = function(RED){
	function LinearRegressionNode(config){
		const path = require('path')
		const utils = require('./utils/utils')

		var node = this;
		//set configurations
		node.file = __dirname + '/LinearRegression.py'
		node.config = {
			savePath: config.savePath || "LinearRegression.sav",
			target: config.target || 'y',
			l2: Number(config.l2) || 0,
			optimizer: Number(config.optimizer),
	...
	RED.nodes.registerType("LinearRegression", LinearRegressionNode)
}
